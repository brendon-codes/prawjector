...
Prawjector is a keyboard-driven project launcher for developers who prefer to stay in the terminal. It opens named, persistent Zellij sessions and creates pre-configured tab layouts for fresh project launches. Reattaching to a live session resumes the exact tabs and shells that are active. Reattaching to an exited session uses Zellij's paused resurrection flow without auto-running saved commands; if Zellij's saved layout has no real tabs, Prawjector removes that stale saved session and starts the configured project fresh.
